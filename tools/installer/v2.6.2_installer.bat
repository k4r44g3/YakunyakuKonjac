...
if %PythonMajorVersion% neq 3 (
    msg * "このアプリケーションを実行するにはPython 3.8以上、3.12以下が必要です。"^

    "Python 3.8以上、3.12以下をインストールしてから再度実行してください。"
    pause
    exit
)
...
@REM マイナーバージョンが12より大きいなら
if %PythonMinorVersion% gtr 12 (
    msg * "このアプリケーションを実行するにはPython 3.8以上、3.12以下が必要です。"^
...
@REM GitリポジトリzipファイルのURL
set RepositoryZipFileURL="https://github.com/%GitUserName%/%RepositoryName%/archive/%TagReference%.zip"
...
curl -#L %RepositoryZipFileURL% -o "%zipFileBaseName%.zip"
@REM エラーが発生したなら(終了コードが0以外なら)
if %errorlevel% neq 0 (
...
@REM バージョン記述ファイルの作成
echo %VersionNumber% > "version.txt"
