...
class Fn:
    ...
    def check_is_python_installed():
        ...
        Returns:
            is_python_installed(bool): Python 3.8がインストールされているかどうか
        """
        # pythonの場所
        PYTHON_PATH = "py"
        ...
        return False  # Python 3.8がインストールされているかどうか
...
class InstallThread:
    ...
    def run(window):
        ...
        if not IS_PRIVATE:
            ...
            # 非公開用
            GIT_URL = "https://github.com/pppp-987/Yakunyakukonjac.git"
            GIT_REPOSITORY_NAME = "Yakunyakukonjac"
        ...
        try:
            ...
            # 仮想環境の作成
            Fn.command_run([PYTHON_PATH, PYTHON_VERSION, "-m", "venv", venv_path])
            ...
            if not os.path.isdir(os.path.join(venv_path, GIT_REPOSITORY_NAME)):
                # gitからクローンする
...
class Main:
    ...
    def run():
        ...
        if not Fn.check_is_python_installed():
            sg.popup("このアプリケーションを実行するにはPython 3.8が必要です。")
            return
