...
def check_file_limits(directory_path, max_size_mb, max_file_count, max_days):
    ...
    Args:
        ...
        max_size_mb (int): 最大保存容量(MB)
        max_file_count (int): 最大保存枚数
        max_days (int): 最大保存期間(日)
    """
    # ファイル名のリストを降順で取得
    ...
    # 指定された制限を超えているかどうかを保存するリスト
    check_file_limit_list = []
    ...
    for file_name in file_name_list:
        if not is_limit_over:
            ...
            if True not in file_limit_info_dict.values():
                ...
                check_file_limit_list.append({file_name: False})
            else:
                # 指定された制限を超えたかどうか
                is_limit_over = True
                # 指定された制限を超えているかどうかを保存
                check_file_limit_list.append({file_name: True})
        else:
            # 指定された制限を超えているなら
            # 指定された制限を超えているかどうかを保存
            check_file_limit_list.append({file_name: True})
...
print(check_file_limits(directory_path, 0.25, 2, 2))
