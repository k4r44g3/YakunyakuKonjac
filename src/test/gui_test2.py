...
class Test:
    ...
    def resize_and_refresh_gui(self, image, user_zoom_scale):
        ...
        # 新しいサイズを計算
        new_image_size = (int(image.size[0] * zoom_scale), int(image.size[1] * zoom_scale))
        # 画像をリサイズ
        resized_img = image.resize(new_image_size, Image.LANCZOS)
        # GUIの画像要素を更新
        self.window["-IMAGE-"].update(data=ImageTk.PhotoImage(resized_img))
        # Columnのスクロール可能領域の更新
        self.window["-COLUMN-"].Widget.canvas.config(scrollregion=(0, 0, new_image_size[0], new_image_size[1]))
