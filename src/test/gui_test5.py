...
class Test:
    ...
    def resize_and_refresh_gui(window, image, fit_zoom_scale, user_zoom_scale):
    ...
    # def window_resize_thread(window):
    #     # 前回のカラムサイズの取得
    #     before_size = window["-COLUMN-"].get_size()

    #     # ウィンドウが閉じられていないなら
    #     while not (window.was_closed()):
    #         # カラムサイズの取得
    #         after_size = window["-COLUMN-"].get_size()
    #         # 前回からカラムサイズが変更されているなら
    ...
    #             column_size = [
    #                 value - window["-COLUMN-"].metadata["scrollable_width"] for value in after_size
    #             ]

    #             # スレッドから、キーイベントを送信
    #             window.write_event_value(key="-window_resize-", value={"column_size": column_size})
    #             Test.a += 1
    #             print(f"a{Test.a}")
    #             # print(f"サブ:{Test.window_resize_thread_stop}")

    #             # Test.window_resize_thread_stop = True
    ...
    def main():
        ...
        layout = [
            [
                sg.Column(
                    ...
                    metadata={
                        "scrollable_width": 15,  # スクロールバー用のスペース
                    },
                )
            ],
            [
                sg.Button("サイズ変更", key="-size-", size=(10, 2)),
                sg.Button("表示", key="-print-", size=(10, 2)),
                sg.Button("変更", key="-window_resize-", size=(10, 2)),
            ],
        ]
        ...
        fit_zoom_scale = Test.get_fit_zoom_scale(image, START_COLUMN_WIDTH, START_COLUMN_HEIGHT)

        # 利用者が変更できる拡大率
        user_zoom_scale = 1

        # 画像のサイズを変更してウィンドウを更新する処理
        Test.resize_and_refresh_gui(window, image, fit_zoom_scale, user_zoom_scale)
        ...
        while True:
            ...
            if event == "-window_resize-":
                # スクロールバーの幅を含まないカラムサイズの計算
                column_size = [
                    value - window["-COLUMN-"].metadata["scrollable_width"]
                    for value in window["-COLUMN-"].get_size()
                ]
                # 画像を与えられた範囲に収まるようにするための拡大率
                fit_zoom_scale = Test.get_fit_zoom_scale(image, column_size[0], column_size[1])
